# Asset manifest

The Drive folder is now link-shared, so the client's own imagery is in place.
17 slots come from Drive, 2 are still openly licensed stock, and 2 remain
designed placeholders. See [SHOT-LIST.md](SHOT-LIST.md) for what each one needs.

## From the client's Drive folder

| Slot | Drive file |
| --- | --- |
| `ba-1-after` | `commercial-after.png` |
| `ba-1-before` | `commercial-before.png` |
| `faq` | `faq-photo.png` |
| `featured-project` | `commercial-1.png` |
| `hero` | `commercial-before-after-split.png` |
| `og-home` | `commercial-1.png` |
| `owner-james` | `owner-consulting.png` |
| `service-maintenance` | `inspections-service.png` |
| `service-metal` | `commercial-4.png` |
| `service-repair` | `commercial-flat-service.png` |
| `service-restoration` | `commercial-2.png` |
| `step-1` | `delivery-truck.png` |
| `step-2` | `inspection.png` |
| `step-4` | `repair-service.png` |
| `system-acrylic` | `commercial-3.png` |
| `system-metal` | `commercial-5.png` |
| `system-silicone` | `commercial-roofing-service.png` |

Two of these matter more than the rest.

**`commercial-before-after-split.png` is the hero**, at the agency's direction. The
composition puts the failed roof behind the headline and the restored roof to the right of
it, so the page argues its own positioning before a word is read.

**`commercial-before.png` and `commercial-after.png` are a genuine matched pair**: the same
building, the same aerial angle, weathered and then coated. That is the one thing a
before/after slider needs and the one thing stock can never supply, so it now drives the
first slider on the page.

## Still openly licensed stock

| Slot | Title | Creator | Licence |
| --- | --- | --- | --- |
| `system-foam` | Raised ceiling, spray foam insulation | Brock Builders | CC BY 2.0 |
| `parallax-fourcorners` | Dripping Springs, Organ Mountains-Desert Peaks, New Mexico | mypubliclands | CC BY 2.0 |

Nothing in Drive covers spray foam or a Four Corners landscape. Both need attribution while
they remain, and both should be replaced before launch.

## Still designed placeholders

- `step-3` (the written scope on site)
- `step-5` (the warranty handover)

Neither has a Drive equivalent.

The before/after section now runs **one** full-width slider on the single matched pair
rather than three. The other two had no matched source, and a row of one real pair beside
two blanks argued less than the real pair does on its own. The `ba-2` and `ba-3` files are
deleted rather than left orphaned. Add sliders back as James supplies matched pairs: the
requirement is the same roof, the same angle, before and after.

## A note on the Drive library

It is AI-generated rather than photographed, and it is predominantly residential: of roughly
100 files, about ten are commercial or flat-roofing, and those ten are now all in use. The
residential shingle, siding and window images are not usable on a commercial-only site.

`Roofing logo.png` in the folder is a generic "YOUR ROOFING COMPANY" template mark, not this
client's logo. The site uses a type-set wordmark instead.

Everything here is still a placeholder for real photography of James's own completed work.
